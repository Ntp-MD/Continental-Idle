...
- Grid translation: A fall cannot be seen on this grid, so translate to plan limits: `drainRunTiles(fixtures → nearest stack) ≤ 20 tiles (10 m)` for small-bore waste and `≤ 40 tiles (20 m)` for large soil runs — **these placeholders are SUPERSEDED by `mep-sizing.md` SV-26, which computes the budget from the §704.1 gradients and a declared 0.15 m floor build-up: 14 tiles at ¼ in/ft (small bore), 28 tiles at ⅛ in/ft (3–6 in laterals). The 20-tile figure is ~40 % too generous and the 40-tile run cannot be bought with one storey of fall at any of the three gradients.** The original heuristic is kept here only as the record of what was assumed before the arithmetic existed, and both sit inside §708's ~100 ft (30.5 m) cleanout-spacing ceiling; while the 40-tile run's own fall (≈21 mm/m on small bore) would consume 0.84 m over 20 m, i.e. more than a floor build-up; `path must be rectilinear with ≤ 1 bend (no bends through tiles that are neither drain-tagged nor shaft)`, and `no drain-tagged tile beneath a bedroom/kitchen tile` unless the room below is accessible ceiling.
