# 🔍 Universal Exhaustive Audit Prompt — Full Monolith Edition (v2)
...
### Derive category priority from what you just detected

Do not treat all 15 categories as equal-effort by default. Immediately after Step 0, classify each category as **High / Standard / Light** priority using this decision rule — not a fixed list:

- Any category whose triggering condition (external boundary, persistence layer, concurrency, rendered UI, etc.) is **present** in the detected project → **High priority**, exhaustive treatment.
- Any category whose triggering condition is **absent** → **Light priority**, may be summarized briefly with a stated reason rather than exhaustively walked.
- Everything else → **Standard**.

State this priority classification as a short table right after your Step 0 paragraph, e.g. `Category 2 (Security): High — project has network-facing entry points`. This keeps effort allocation self-derived from the actual codebase instead of assumed in advance, so the same prompt stays correct whether the target is a public API or an offline CLI tool.
...
### If the codebase exceeds comfortable processing in one pass

Judge this by whether you can hold the full file set in working context without losing earlier detail — not by a fixed line count, since this varies by model and session.

If it does not fit comfortably:

1. Process by logical module or directory boundary, one at a time.
2. Output findings per module as you go, tagged with the module name.
3. State explicitly which modules were processed at reduced depth, and why.
4. Only write the Architectural Debt section (Category 15) after all modules are covered — it depends on the full picture.

Never silently drop a module. If something is out of reach, say so and name it.
...
**Depth allocation rule:** trace entry points reachable from unvalidated external input to full depth first. Internal-only call paths (no external boundary anywhere upstream) can be traced at lower depth once the external-facing paths are covered — state when you've done this.
...
**Auto-applicability rule:** Before auditing each category, write one line stating whether it is **Applicable**, **Partially Applicable**, or **N/A** for this project, with a one-sentence reason. Then proceed accordingly — skip N/A categories entirely but still declare them. Never silently omit a category. For **Light priority** categories (per your Step 0 classification), a short paragraph with the most important 1-3 findings is sufficient — state that you summarized and why.
...
> - Before citing any line range, verify it against the actual file content via a read/grep tool call if one is available — do not estimate line numbers from memory of pasted or scrolled-past code
...
Use these fixed bands for the **Confidence** field on every finding — do not assign a free-floating number:

| Band          | Meaning                                                                                                                                                                                  |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **90–100%**   | Exact line(s) cited, logic traced end-to-end from source to sink, no assumption made about unseen code                                                                                   |
| **60–89%**    | Pattern is clearly present in the cited code, but full confirmation would require runtime execution or data you don't have access to                                                     |
| **Below 60%** | Suspicious pattern only — insufficient evidence to assert as a bug. Route this to "Requires Manual Verification" in the summary instead of listing it as a confirmed or probable finding |

This keeps confidence tied to something checkable (how much of the chain you actually traced), not a gut-feel number that will drift between runs or between models.
...
| **Confidence**     | 0–100% (use the anchor bands above)                                  |
...
### Worked Example (reference only — replace with real findings from the target codebase)

This shows what a fully-completed finding looks like, so the shape above isn't just a schema description. This example is intentionally stack-agnostic; it applies whether the audited code is a frontend, a backend service, or a CLI tool.

---

#### [Severity: High] — Unhandled promise rejection on external call

| Field              | Value                                                                                                      |
| ------------------ | ---------------------------------------------------------------------------------------------------------- |
| **Category**       | 3 — Concurrency & Async                                                                                    |
| **Location**       | `example-service.ts` lines 88–94                                                                           |
| **Confidence**     | 95% — exact call site cited, no `.catch()` or `try/catch` anywhere in the call chain up to the entry point |
| **Exploitability** | Moderate — triggered whenever the external call fails, not attacker-controlled directly                    |
| **Blast Radius**   | Feature-level — the calling flow silently stalls; no crash, no user feedback                               |

**Description:**
An external call (network request, subprocess, or similar) is awaited without any rejection handling. If it fails, the promise rejection is unhandled: in some runtimes this crashes the process, in others it fails silently with no user-facing error.

**Root Cause:**
The `await someExternalCall(...)` at line 91 has no surrounding `try/catch`, and the calling function itself is not wrapped by a caller that handles rejection either.
...
async function loadData() {
  const result = await someExternalCall(input); // unhandled rejection path
  return result;
}
...
async function loadData() {
  try {
    ...
    logError("loadData failed", { input, err });
    throw new LoadDataError("Failed to load data", { cause: err });
  }
}
```

**Verification Step:**
Force `someExternalCall` to reject (e.g. disconnect network, mock a failure) and observe whether the failure is caught, logged, and surfaced — or whether it propagates unhandled.
...
- [ ] Category priority table (High / Standard / Light) derived and stated right after Step 0
...
- [ ] Light-priority categories summarized with a stated reason, not silently thin
...
- [ ] Every finding has: Category, Location, Confidence % (from the anchor bands), Exploitability, Blast Radius, Proposed Fix, Verification Step
- [ ] Every cited line range was checked against actual file content via tool, not recalled from memory
- [ ] If the codebase required chunked processing, every module's coverage depth is stated
...
9. **Effort follows risk, not habit** — depth of treatment per category is derived from what Step 0 detected, not applied uniformly by default

---

_Perform all 3 passes. Detect project type. Derive category priority. Then write the report._
