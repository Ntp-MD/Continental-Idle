...
export function validateLayoutData(data: unknown): LayoutData | null {
  ...
  for (const floor of layout.floors) {
    ...
    if (floor.zones !== undefined && (!Array.isArray(floor.zones) || floor.zones.some(zone => zone.tags !== undefined && (!Array.isArray(zone.tags) || zone.tags.some(tag => typeof tag !== 'string'))))) return null
    if (floor.rooms.some(room => room.tags !== undefined && (!Array.isArray(room.tags) || room.tags.some(tag => typeof tag !== 'string')))) return null
    if (floor.objects.some(object => object.customProps?.tags !== undefined && (!Array.isArray(object.customProps.tags) || object.customProps.tags.some(tag => typeof tag !== 'string')))) return null
  }
...
export function validateLayoutIntegrity(layout: LayoutData): string[] {
  const issues: string[] = []
