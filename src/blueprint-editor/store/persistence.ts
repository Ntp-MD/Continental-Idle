...
const SAVED_LAYOUT: LayoutData = ${JSON.stringify(data)}
`
...
export function syncToGame(): boolean {
  try {
    const synced: Record<string, {
      ...
      objects: Array<{ id: string; type: string; x: number; y: number; w: number; h: number; rotation: number; walkable: boolean; entranceRequired: boolean; walkableGrid?: boolean[][]; tileStates?: string[][]; roomId?: string; label?: string; tags?: string[] }>
    ...
    localStorage.setItem(SYNC_KEY, JSON.stringify({ version: 2, floors: synced, timestamp: Date.now() }))
    return true
  } catch (e) {
