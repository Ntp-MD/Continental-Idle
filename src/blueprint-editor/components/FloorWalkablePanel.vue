...
function syncEdgesFromSegments(): void {
  const edges: TileEdges[][] = Array.from({ length: rows.value }, () => Array.from({ length: cols.value }, () => ({}) as TileEdges));
...
function resetDraft(): void {
  ...
  wallSegments.value = (props.floor?.objects ?? [])
    .filter((object) => object.isWall && [object.x1, object.y1, object.x2, object.y2].every((value) => typeof value === "number"))
    .map((object) => {
  ...
  <ModalShell :open="open" title="Walkable Setting" dialog-class="walkable__dialog" @close="close">
    <div class="modal__body">
      <div class="form__row form__row--tight form__row--wrap" role="toolbar" aria-label="Walkable setting tools">
        ...
        <button type="button" :class="{ 'flag--warning': activeMode === 'door' }" @click="setMode('door')">Door / Door</button>
      ...
      <div class="walk__grid" :style="gridStyle" role="grid" :aria-label="`${buildingCols} by ${buildingRows} walkable grid`">
        <template v-for="rowIndex in buildingRows" :key="`walk-row-${rowIndex}`">
          ...
          >
            <span v-if="gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.top" class="walk__edge walk__edge--top" :class="{ 'walk__edge--door': gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.doorTop }"></span>
            <span v-if="gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.right" class="walk__edge walk__edge--right" :class="{ 'walk__edge--door': gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.doorRight }"></span>
            <span v-if="gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.bottom" class="walk__edge walk__edge--bottom" :class="{ 'walk__edge--door': gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.doorBottom }"></span>
            <span v-if="gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.left" class="walk__edge walk__edge--left" :class="{ 'walk__edge--door': gridEdges[buildingStartRow + rowIndex - 1]?.[buildingStartCol + colIndex - 1]?.doorLeft }"></span>
          </button>
        </template>
      </div>
