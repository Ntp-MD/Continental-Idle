...
export function useCanvasDragDrop(
		...
		if (svgEl) {
			...
			if (inside && ghost) {
				...
				try {
					...
					console.error('addObject failed', err)
				}
			}
		}
		endAssetDrag()
	...
	async function onRoomTemplateMouseUp(e: MouseEvent): Promise<void> {
		...
		if (svgEl) {
			...
			if (inside && ghost && roomTemplateValid.value) {
				try {
					...
					toast.error(err instanceof Error ? err.message : 'Failed to place room template')
					console.error('addRoomFromTemplate failed', err)
				}
			}
		}
		endRoomTemplateDrag()
