  ...
  async function onWindowMouseUpForDrag(e: MouseEvent) {
    ...
    if (svgEl) {
      ...
      if (inside && ghost) {
        const p = opts.localPoint(e)
        if (!p) return
        await opts.store.addObject(assetId, p.x - ghost.w / 2, p.y - ghost.h / 2).catch((e: unknown) => {
          toast.error(e instanceof Error ? e.message : 'Failed to place object')
  ...
  async function onRoomTemplateMouseUp(e: MouseEvent) {
    ...
    if (svgEl) {
      ...
      if (inside && ghost && roomTemplateValid.value) {
        await opts.store.addRoomFromTemplate(templateId, ghost.x, ghost.y).catch((e: unknown) => {
          toast.error(e instanceof Error ? e.message : 'Failed to place room template')
          console.error('addRoomFromTemplate failed', e)
        })
      }
    }
    endRoomTemplateDrag()
