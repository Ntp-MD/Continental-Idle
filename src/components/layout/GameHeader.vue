...
const currencies = computed<CurrencyItem[]>(() => [
  { label: "Gold", value: currency.value, show: true },
  { label: "GC", value: goldenCoins.value, color: "#c9a84c", show: goldenCoins.value !== "0" },
  { label: "RM", value: royalMarks.value, color: "#b8860b", show: royalMarks.value !== "0" },
  { label: "Income", value: income.value, cls: "game__currency__income", show: true },
  { label: "Idle", value: inactiveIncome.value, cls: "game__currency__income", show: inactiveIncome.value !== "0/s" },
  { label: "Favor", value: favor.value, cls: "game__currency__favor", bonus: prestigeMult.value > 0 ? `+${prestigeMult.value}%` : undefined, show: true },
  { label: "Prestige", value: String(prestige.value), show: true },
  { label: "Rep", value: String(reputation.value), show: true },
  { label: "Guests", value: `${satisfaction.value}%`, show: true },
  { label: "Perm", value: `+${permBonus.value}%`, cls: "game__currency__income", show: permBonus.value > 0 },
  { label: "Debt", value: `${debtTotal.value} (${debtCount.value})`, cls: "game__currency__debt", warning: debtWarning.value, show: debtCount.value > 0 },
  { label: "Grace", value: `${graceMinutes.value}m`, cls: "game__currency__grace", show: graceMinutes.value > 0 },
]);
...
<template>
  <header class="game">
    ...
    <div class="game__currencies">
      <div v-for="c in currencies" :key="c.label" v-show="c.show" class="game__currency" :class="c.cls">
        <span class="game__currency__label">{{ c.label }}</span>
        <span class="game__currency__value" :style="c.color ? { color: c.color } : undefined">{{ c.value }}</span>
        <span v-if="c.bonus" class="game__bonus">{{ c.bonus }}</span>
        <span v-if="c.warning" class="game__currency__warning">?</span>
      </div>
