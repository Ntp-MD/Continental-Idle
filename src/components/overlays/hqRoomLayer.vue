...
import { SVG_W, SVG_H, GOLD, GOLD_DIM, GOLD_DARK, BG_DARK, BG_DARKER, BG_CORRIDOR, SYNCED_CANVAS, getRoomsOnFloor, getObjectsOnFloor, getRoomFurniture, FLOOR_NAMES, ELEVATOR_POS, CORRIDOR_LAYOUT, DOOR_LAYOUT, DOOR_WIDTHS } from "./hqLayout";
...
const tileSize = SYNCED_CANVAS.tileSize > 1 ? SYNCED_CANVAS.tileSize : 25;
const streetBorderPx = STREET_TILES * tileSize;
const sidewalkW = 2 * tileSize;
const roadW = 4 * tileSize;
const buildingX = streetBorderPx;
const buildingY = streetBorderPx;
const buildingW = SVG_W - streetBorderPx * 2;
const buildingH = SVG_H - streetBorderPx * 2;
</script>

<template>
  <g v-if="props.unlocked">
    <!-- Street border: sidewalk + road + lane markings -->
    <g class="hq__street" style="pointer-events: none">
      <!-- Outer sidewalk (2 tiles, all sides) -->
      <rect x="0" y="0" :width="SVG_W" :height="sidewalkW" fill="#1a1a1a" />
      <rect x="0" :y="SVG_H - sidewalkW" :width="SVG_W" :height="sidewalkW" fill="#1a1a1a" />
      <rect x="0" y="0" :width="sidewalkW" :height="SVG_H" fill="#1a1a1a" />
      <rect :x="SVG_W - sidewalkW" y="0" :width="sidewalkW" :height="SVG_H" fill="#1a1a1a" />

      <!-- Road (4 tiles, all sides) -->
      <rect :x="sidewalkW" :y="sidewalkW" :width="SVG_W - sidewalkW * 2" :height="roadW" fill="#0d0d0d" />
      <rect :x="sidewalkW" :y="SVG_H - sidewalkW - roadW" :width="SVG_W - sidewalkW * 2" :height="roadW" fill="#0d0d0d" />
      <rect :x="sidewalkW" :y="sidewalkW" :width="roadW" :height="SVG_H - sidewalkW * 2" fill="#0d0d0d" />
      <rect :x="SVG_W - sidewalkW - roadW" :y="sidewalkW" :width="roadW" :height="SVG_H - sidewalkW * 2" fill="#0d0d0d" />

      <!-- Road lane markings (dashed center lines) -->
      <line :x1="sidewalkW" :y1="sidewalkW + roadW / 2" :x2="SVG_W - sidewalkW" :y2="sidewalkW + roadW / 2" :stroke="GOLD_DARK" stroke-width="0.5" stroke-dasharray="6 4" opacity="0.6" />
      <line :x1="sidewalkW" :y1="SVG_H - sidewalkW - roadW / 2" :x2="SVG_W - sidewalkW" :y2="SVG_H - sidewalkW - roadW / 2" :stroke="GOLD_DARK" stroke-width="0.5" stroke-dasharray="6 4" opacity="0.6" />
      <line :x1="sidewalkW + roadW / 2" :y1="sidewalkW" :x2="sidewalkW + roadW / 2" :y2="SVG_H - sidewalkW" :stroke="GOLD_DARK" stroke-width="0.5" stroke-dasharray="6 4" opacity="0.6" />
      <line :x1="SVG_W - sidewalkW - roadW / 2" :y1="sidewalkW" :x2="SVG_W - sidewalkW - roadW / 2" :y2="SVG_H - sidewalkW" :stroke="GOLD_DARK" stroke-width="0.5" stroke-dasharray="6 4" opacity="0.6" />
    </g>

    <!-- Building outer walls (within street border) -->
    <rect :x="buildingX" :y="buildingY" :width="buildingW" :height="buildingH" :fill="BG_DARKER" :stroke="GOLD_DIM" stroke-width="2" />
    <rect :x="buildingX + WALL_THICKNESS" :y="buildingY + WALL_THICKNESS" :width="buildingW - WALL_THICKNESS * 2" :height="buildingH - WALL_THICKNESS * 2" :fill="BG_DARK" :stroke="GOLD_DARK" stroke-width="0.5" />
    ...
    <g v-for="(seg, i) in CORRIDOR_LAYOUT[props.floor] ?? []" :key="'corr_' + i">
      ...
      <!-- Corridor label -->
      <text v-if="seg.label" :x="seg.vertical ? seg.x + seg.w / 2 : seg.x + seg.w / 2" :y="seg.vertical ? seg.y + seg.h / 2 + 3 : seg.y + seg.h / 2 + 3" :text-anchor="'middle'" :font-size="7" :fill="GOLD_DARK" :letter-spacing="2" :transform="seg.vertical ? `rotate(-90 ${seg.x + seg.w / 2} ${seg.y + seg.h / 2 + 3})` : undefined" font-family="Georgia,serif">{{ seg.label }}</text>
    </g>
    ...
    <g v-for="room in getRoomsOnFloor(props.floor)" :key="room.id">
      ...
      <text v-if="!room.visual && props.buildingLevels[room.id] !== undefined && props.buildingLevels[room.id] > 0" :x="room.x + room.w - 12" :y="room.y + 16" text-anchor="end" font-family="Georgia,serif" font-size="10" :fill="GOLD">Lv.{{ props.buildingLevels[room.id] }}</text>
      <!-- Visual only badge -->
      <text v-if="room.visual" :x="room.x + room.w / 2" :y="room.y + room.h / 2 + 10" text-anchor="middle" font-family="Georgia,serif" font-size="7" :fill="GOLD_DARK" letter-spacing="2">VISUAL ONLY</text>
    </g>
    ...
    <g v-for="(door, i) in DOOR_LAYOUT[props.floor] ?? []" :key="'door_' + i">
      ...
      <rect v-if="door.side === 'top' || door.side === 'bottom'" :x="door.x - DOOR_WIDTHS[door.category] / 2" :y="door.y - 3" :width="DOOR_WIDTHS[door.category]" :height="6" :fill="BG_CORRIDOR" />
      <rect v-else :x="door.x - 3" :y="door.y - DOOR_WIDTHS[door.category] / 2" :width="6" :height="DOOR_WIDTHS[door.category]" :fill="BG_CORRIDOR" />
      <!-- Door swing arc (inward opening) -->
      <template v-if="door.category === 'sliding'">
        <!-- Sliding door: two parallel lines indicating panels -->
        <line v-if="door.side === 'top' || door.side === 'bottom'" :x1="door.x - DOOR_WIDTHS[door.category] / 2" :y1="door.y" :x2="door.x + DOOR_WIDTHS[door.category] / 2" :y2="door.y" :stroke="GOLD_DIM" stroke-width="1.5" />
        <line v-if="door.side === 'left' || door.side === 'right'" :x1="door.x" :y1="door.y - DOOR_WIDTHS[door.category] / 2" :x2="door.x" :y2="door.y + DOOR_WIDTHS[door.category] / 2" :stroke="GOLD_DIM" stroke-width="1.5" />
      ...
      <template v-else>
        ...
        <path v-if="door.side === 'top'" :d="`M ${door.x - DOOR_WIDTHS[door.category] / 2} ${door.y} A ${DOOR_WIDTHS[door.category]} ${DOOR_WIDTHS[door.category]} 0 0 1 ${door.x + DOOR_WIDTHS[door.category] / 2} ${door.y + 4}`" fill="none" :stroke="GOLD_DIM" stroke-width="0.8" />
        <path v-if="door.side === 'bottom'" :d="`M ${door.x - DOOR_WIDTHS[door.category] / 2} ${door.y} A ${DOOR_WIDTHS[door.category]} ${DOOR_WIDTHS[door.category]} 0 0 0 ${door.x + DOOR_WIDTHS[door.category] / 2} ${door.y - 4}`" fill="none" :stroke="GOLD_DIM" stroke-width="0.8" />
        <path v-if="door.side === 'left'" :d="`M ${door.x} ${door.y - DOOR_WIDTHS[door.category] / 2} A ${DOOR_WIDTHS[door.category]} ${DOOR_WIDTHS[door.category]} 0 0 1 ${door.x + 4} ${door.y + DOOR_WIDTHS[door.category] / 2}`" fill="none" :stroke="GOLD_DIM" stroke-width="0.8" />
        <path v-if="door.side === 'right'" :d="`M ${door.x} ${door.y - DOOR_WIDTHS[door.category] / 2} A ${DOOR_WIDTHS[door.category]} ${DOOR_WIDTHS[door.category]} 0 0 0 ${door.x - 4} ${door.y + DOOR_WIDTHS[door.category] / 2}`" fill="none" :stroke="GOLD_DIM" stroke-width="0.8" />
        <!-- Door leaf line -->
    ...
    <g v-if="(CORRIDOR_LAYOUT[props.floor]?.length ?? 0) > 0 && (ELEVATOR_POS[props.floor]?.r ?? 0) > 0">
    ...
    <!-- Floor name -->
    <text :x="SVG_W / 2" :y="SVG_H - 10" text-anchor="middle" font-family="Georgia,serif" font-size="9" :fill="GOLD_DIM" letter-spacing="4">{{ (FLOOR_NAMES[props.floor] ?? props.floor).toUpperCase() }}</text>
  </g>
  <g v-else>
