...
export function getRouteById(routeId: string): SupplyRoute | undefined {
	return gameState.get().supplyRoutes.find(r => r.id === routeId)
}
...
export function tickSupplyRoutes(): void {
	...
	state.supplyRoutes.forEach(route => {
		...
		const income = route.incomePerTick * stabilityMult * getTotalIncomeMultiplier()
