import type { BranchId, AssassinEntry, Rarity } from '@/types'
import { ASSASSIN_MAP } from '@/data/assassins'
import { getTraitMultiplier } from '@/data/traits'
import { gameState } from './gameState'
import { eventBus } from './eventBus'
import { getRoyalLoyaltyDecayReduction, getSovereignBuffMult } from './royalManager'
...
function rollAssassinStats(rarity: Rarity) {
	return rollStats(rarity, { statBudgetBonus: 4, statMinBonus: 1, statMaxBonus: 2 })
}
...
export function getAssassinXpToNext(level: number): number {
	return getXpToNext(level, 200, 1.4)
}
...
export function getAssassinLevelUpCost(assassinTypeId: string, newLevel: number): number {
	...
	return getLevelUpCost(def.hireCost, 0.15, 1.4, newLevel - 1)
}
	...
	state.worldMap.unlockedBranches.forEach(branchId => {
		...
		Object.values(branch.assassins).forEach(assassin => {
			...
			if (assassin.assignedBranch && assassin.assignedBranch !== branchId) {
				...
				const isSovereignNoDecay = sovereignManager.hasActiveDecree('loyaltyBoost') && sovereignManager.getActiveDecreeMult('loyaltyBoost') === -1
				if (!isSovereignNoDecay) {
