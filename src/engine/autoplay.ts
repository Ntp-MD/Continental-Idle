...
import { purchaseBuilding, getBranchIncomePerSecond, getBuildingCost, getBuildingIncome, setSuppressUIEvents } from './incomeEngine'
import { hireStaff, assignStaff, confirmLevelUp, isStaffUnlocked } from './staffManager'
import { getTotalDebt, repayAllDebts } from './debtManager'
import { hireAssassin, isAssassinUnlocked, assignAssassin, sendAssassinToAttack, confirmAssassinLevelUp, getAssassinCombatDamage, hasEnforcer, hasHighTableEnforcer, cancelAssassinAttack, lendAssassin, recallAssassin } from './assassinManager'
import { callVisitor, hireVisitor, dismissVisitor, royalMarkScroll, canUseRoyalMarkScroll } from './visitorManager'
import { getRarityCostMult } from '@/data/rarity'
import { initiateTakeover, canInitiateTakeover, getTakeoverCost } from './takeoverManager'
import { canLayLow, layLow, canHostEvent, hostEvent, canBribeOfficial, bribeOfficial, canGoldenCoinIncomeBoost, goldenCoinIncomeBoost } from './actionsManager'
import { canEstablishRoute, establishRoute, stabilizeRoute, getSupplyRoutes, getStabilizeCost, canHijackRoute, hijackRoute, getHijackableRoutes } from './supplyRouteManager'
...
import { upgradeSkill, getExtraStaffSlots } from './skillManager'
import { purchaseUpgrade, UPGRADES } from './upgradeManager'
import { doPrestige, getPrestigeFavor } from './prestigeManager'
import { purchaseRoyalBuilding, canUpgradeRoyalSkill, upgradeRoyalSkill, canRoyalPrestige, getRoyalPrestigeMarks, doRoyalPrestige, getRoyalAffordableLevels } from './royalManager'
    ...
    if (Object.keys(branch.staff).length < maxStaff) {
      ...
      const candidates = STAFF_TYPES
        ...
        .map(def => {
          ...
          if (def.id === 'adjudicator') defensiveValue = 80
