...
import { STAFF_MAP, getVeteranPerk } from '@/data/staff'
...
import { getRoyalBuildingsIncome } from './royalManager'
	...
	Object.values(branchState.staff).forEach(staff => {
		if (staff.assignedTo === buildingId) {
			...
			if (staffDef) {
				...
				if (staff.veteran) traitIncomeMult *= getVeteranPerk(staff.veteranPerk).incomeMult
			}
		}
	})
...
export function getTotalIncomePerSecond(): number {
	const state = gameState.get()
	let total = 0
	state.worldMap.unlockedBranches.forEach(branchId => {
		const cached = getCachedBranchIncome(branchId)
		if (cached !== null) {
			total += cached
		} else {
			const result = _computeBranchIncomePerSecond(branchId)
			if (_tickCache) _tickCache.set(branchId, result)
			total += result
		}
	})
	return total
...
function _computeBranchIncomePerSecond(id: BranchId): number {
	...
	if (hasFreeze && !hasEnforcer(id)) {
		if (getBartenderFreezeImmune(id)) {
			...
			return barIncome * prestigeMult * hqMult * satMult * repMult * buffMult * incomeMultiplier * permBonus * conciergeBonus
		}
		return 0
	}
...
export function getBuildingCost(branchState: BranchState, buildingId: string, count: number = 1): number {
	...
	return computeBuildingCost(def.baseCost, def.costGrowth, bState.level, count)
}
...
export function getAffordableLevels(branchState: BranchState, buildingId: string): number {
	...
	const currentLevel = branchState.buildings[buildingId]?.level || 0
