...
- **Git history is off-limits.** Never inspect or restore committed state - no `checkout` / `restore` / `reset` / `diff` / `log` against HEAD or any commit, no stash. Work with working-tree files only, through Read/Edit tools. Reverting means editing the file back by hand, never pulling content out of history.
