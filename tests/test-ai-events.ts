...
import {
	...
	generateAIEvent,
	type AIEventType,
} from '../src/engine/aiOwnerManager'
import { AI_TEMPERAMENTS, getAIName, getAITemperamentForBranch, getTemperamentDef } from '../src/data/aiOwners'
import {
	AI_EVENT_TEMPLATES,
	getAIEventTemplate,
	buildAIEventDefinition,
} from '../src/data/aiEvents'
...
ok((invalidOwner.branchId as string) === 'nonexistent', 'createAIOwner invalid branch still sets branchId')
...
for (let i = 0; i < 200; i++) {
	const pick = pickAIEvent(weakOwner, 10000)
...
aiTickState.branches.bangkok.excommunicadoGraceUntil = 0
	; (eventEngine as any).lastEventTimes = new Map()
	; (eventEngine as any).tickCount = 2
