...
import {
	...
	type PersistencePort, type SyncPort,
...
test('store save lifecycle: success, revert on failure, false, payload-cap, single writer', async () => {
	const harness = createRecordingPort()
	const store = createBlueprintStore({ persistence: harness.port, sync, seed: await defaultSeed() })
	...
	assert.equal(
		...
		'Failed to save blueprint data - changes reverted (disk is full)',
		'the failed save names the reason and says the changes were reverted',
	)
...
test('local-first port: storage round-trip + failure modes', async () => {
	const harness = createRecordingPort()
	const store = createBlueprintStore({ persistence: harness.port, sync, seed: await defaultSeed() })
	await store.save()
	const payload = harness.saves[0]
...
test('HTTP port: retry, 413, read-back verification', async () => {
	const harness = createRecordingPort()
	const store = createBlueprintStore({ persistence: harness.port, sync, seed: await defaultSeed() })
	await store.save()
	const payload = harness.saves[0]
...
test('a reload that lands mid-command must not discard the committed write', async () => {
	const stored: { body: string | null } = { body: null }
	let gate: (() => void) | null = null
	const port: PersistencePort = {
		async load() {
			return stored.body ? (JSON.parse(stored.body) as BlueprintDataFile) : null
		},
		async save(data) {
			await new Promise<void>(resolve => {
				gate = resolve
			})
			// Serialized like every real port: the payload must not alias live state.
			stored.body = JSON.stringify(data)
			return true
		},
	}
	const openGate = async () => {
		for (let i = 0; i < 20 && !gate; i++) await Promise.resolve()
		const pending = gate
		gate = null
		pending?.()
	}

	const store = createBlueprintStore({ persistence: port, sync, seed: await defaultSeed() })
	const primed = store.save()
	await openGate()
	await primed
	const before = store.state.layout.floors.length

	const added = store.addFloor()
	await new Promise(resolve => setTimeout(resolve, 0))
	const reloaded = store.reloadEditorData()
	await openGate()
	await Promise.all([added, reloaded])

	assert.equal(
		store.state.layout.floors.length,
		before + 1,
		'reloadEditorData() waited for the in-flight addFloor() instead of overwriting it',
	)
})
