...
test('boundary hardening: placement, tags, serialize, resolve, normalize', () => {
...
test('SVG color convention: body vars applied, hollow details untouched', () => {
...
test('canvas config pipeline: specs, round-trip, strict/lenient, grid caps', () => {
	const canvasKeys = Object.keys(CANVAS_FIELD_SPECS).sort()
	assert.deepEqual(canvasKeys, ['bgColor', 'gridColor', 'height', 'labelColor', 'streetMarkingColor', 'streetRoadColor', 'streetSidewalkColor', 'tileSize', 'wallColor', 'width'])

	const sampleCanvas: Required<CanvasConfig> = { width: 100, height: 50, tileSize: 25, bgColor: '#000000', labelColor: '#cccccc', wallColor: '#ff0000', gridColor: '#333333', streetSidewalkColor: '#21262d', streetRoadColor: '#161b22', streetMarkingColor: '#6e7681' }
...
test('legacy wall keys drop on objects', () => {
...
test('InteractSpot union (stand/edge/post) + task.post round-trip', () => {
...
test('tile door states round-trip', () => {
...
test('resolveDefaultWalkable is the single default path', () => {
