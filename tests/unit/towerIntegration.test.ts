...
test('real workspace seed stays deployable end-to-end: migrate, layout build, portals, spawn, 120 ticks', () => {
	...
	assert.equal(agents.length, spawned, `no agent lost during ticks (${layout.floors.length} floors, ${spawned}/${expected} agents, ${portals.length} portal routes)`)
