...
let originAssets: Awaited<ReturnType<typeof seedOriginAssets>>

beforeAll(async () => {
	originAssets = await seedOriginAssets()
})
...
test('migrate preserves floor count and npc config', () => {
...
test('migrate rejects invalid input', () => {
...
test('missing npcConfig produces an empty config; top-level npcConfig survives', () => {
...
test('editorSettings survive migration field by field', () => {
...
test('partial role salvage: bad roles dropped, good roles kept', () => {
...
test('objects with unknown asset type are filtered out', () => {
...
test('objects with unusable placement are dropped, not corrupted', () => {
	const validAsset = originAssets[0]
	if (!validAsset) throw new Error('test requires at least one origin asset')
...
test('legacy wall keys are stripped without dropping the object', () => {
	const validAsset = originAssets[0]
	if (!validAsset) throw new Error('test requires at least one origin asset')
...
test('floors missing an objects array get empty objects', () => {
...
test('normalizeNpcConfig salvages partial config', () => {
...
test('appearance salvage is lenient: no role dropped for a bad appearance', () => {
...
test('migration and appearance normalization round-trips are stable', () => {
...
test('golden fixture round-trips and rejects bad versions', () => {
...
test('persisted w:0/h:0 objects re-derive size from the live asset on ingress', () => {
	const validAsset = originAssets[0]
	if (!validAsset) throw new Error('test requires at least one origin asset')
