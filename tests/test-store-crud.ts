...
async function main(): Promise<void> {
	...
	await check('undo() restores a deleted asset and its placed instances', async () => {
		restore(baseline)
		installTestAsset()
		const floor = state.layout.floors[0]
		state.currentFloorId = floor.id
	...
	await check('undo() depth caps at 4 steps', async () => {
		restore(baseline)
		for (let i = 0; i < 6; i++) await addFloor()
		assert.equal(state.layout.floors.length, 7, 'six floors added')
		for (let i = 0; i < 4; i++) assert.equal(await store.undo(), true, `undo step ${i + 1} applies`)
		assert.equal(state.layout.floors.length, 3, 'four undos rolled back four floors')
		assert.equal(await store.undo(), false, 'fifth undo refused - history capped at 4')
		assert.equal(state.layout.floors.length, 3, 'floors unchanged after refused undo')
	...
	await check('save() prunes dangling npc references', async () => {
		restore(baseline)
		const npc = state.layout.npcConfig
		assert.ok(npc && npc.roles.length > 0, 'baseline npc config with a role is required')
	...
	await check('save() strips renamed spot posts but keeps the asset binding', async () => {
		restore(baseline)
		const asset = installTestAsset()
	...
	await check('addSpawnZone() creates a labeled zone from a rect', async () => {
		restore(baseline)
		const floor = state.layout.floors[0]
	...
	await check('reorderFloors() swaps floor order and keeps the current floor', async () => {
		restore(baseline)
		...
		assert.equal(await store.reorderFloors(0, 5), false, 'out-of-range reorder rejected')
		assert.equal(await store.reorderFloors(1, 1), false, 'no-op reorder rejected')
	})

	await check('duplicateAsset() copies the palette entry without touching instances', async () => {
		restore(baseline)
		installTestAsset()
		const floor = state.layout.floors[0]
	...
	await check('setStreetWidth() repaints the street ring walkable', async () => {
		restore(baseline)
		state.layout.canvas = { width: 1600, height: 1200, tileSize: 25 }
		const floor = state.layout.floors[0]
		resetWalkable(floor)
	...
	await check('removeTag() strips spot posts in the same namespace', async () => {
		restore(baseline)
		const asset = installTestAsset()
		asset.interactSpots = [{ kind: 'stand', x: 1, y: 1, post: 'grill-post' }]
	...
	await check('save() prunes trigger rates for unused tags', async () => {
		restore(baseline)
		const npc = state.layout.npcConfig
		assert.ok(npc && npc.roles.length > 0, 'baseline npc config with a role is required')
	...
	await check('save() strips the dead spawnRule.count field', async () => {
		restore(baseline)
		const npc = state.layout.npcConfig
		assert.ok(npc && npc.roles.length > 0, 'baseline npc config with a role is required')
